...
public struct SubRoomData
{
    ...
    public SubRoomData(int[,] fullGrid, bool ifUp, bool ifRight)
    {
        grid = new int[5, 4];
    ...
    public static int[,] RecomposeRoom(List<SubRoomData> shuffledSubRooms, ExitConstraint requiredExit)
    {
        if (shuffledSubRooms == null || shuffledSubRooms.Count < 4)
        {
            throw new UnityException("Not enough SubRoomData to generate a new room.");
        }

        List<SubRoomData> selectedSubRooms = new List<SubRoomData>(4); // 存储选中的4个片段
        ...
        // **遍历所有 SubRoomData，按需求选择**
        SubRoomData? leftUp = null, rightUp = null, leftDown = null, rightDown = null;
        ...
        if (!leftUp.HasValue || !rightUp.HasValue || !leftDown.HasValue || !rightDown.HasValue)
        {
            throw new UnityException("Failed to find suitable SubRoomData pieces to match the required exits.");
        }
